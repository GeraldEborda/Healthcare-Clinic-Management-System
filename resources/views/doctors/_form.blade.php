...
<form method="POST" action="{{ $editing ? route('doctors.update', $doctor) : route('doctors.store') }}">
    ...
    <label>Time Slots <input type="text" name="time_slots_input" value="{{ old('time_slots_input', isset($doctor) ? implode(', ', $doctor->time_slots ?? []) : '') }}" placeholder="09:00-12:00, 13:00-17:00"></label>
    <label>Qualifications <textarea name="qualifications">{{ old('qualifications', $doctor->qualifications ?? '') }}</textarea></label>
    <div class="actions">
