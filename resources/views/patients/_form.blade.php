...
<form method="POST" action="{{ $editing ? route('patients.update', $patient) : route('patients.store') }}">
    ...
    <label>Emergency Contact <input type="text" name="emergency_contact" value="{{ old('emergency_contact', $patient->emergency_contact ?? '') }}"></label>
    <label>Address <textarea name="address">{{ old('address', $patient->address ?? '') }}</textarea></label>
    <label>Medical History <textarea name="medical_history">{{ old('medical_history', $patient->medical_history ?? '') }}</textarea></label>
    <div class="actions">
